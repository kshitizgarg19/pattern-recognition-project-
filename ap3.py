...
st.title("Urban Land Cover Prediction")
import streamlit as st
import pandas as pd
import numpy as np
from sklearn.ensemble import RandomForestRegressor
...
for col in selected_features:
    user_input[col] = st.number_input(f"Enter {col}:", value=float(X_train[col].mean()), min_value=float(X_train[col].min()), max_value=float(X_train[col].max()))
